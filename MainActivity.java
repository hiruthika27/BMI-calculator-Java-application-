...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.weight_cardView) {
        } else if (id == R.id.weight_btn_inc) {
            if (weightCounter < 700)
                weightCounter++;
            countWeight = Integer.toString(weightCounter);
            weightCounterText.setText(countWeight);
        } else if (id == R.id.weight_btn_dec) {
            if (weightCounter > 0)
                weightCounter--;
            countWeight = Integer.toString(weightCounter);
            weightCounterText.setText(countWeight);
        } else if (id == R.id.age_cardView) {
        } else if (id == R.id.age_btn_inc) {
            if (ageCounter < 150)
                ageCounter++;
            countAge = Integer.toString(ageCounter);
            ageCounterText.setText(countAge);
        } else if (id == R.id.age_btn_dec) {
            if (ageCounter > 1)
                ageCounter--;
            countAge = Integer.toString(ageCounter);
            ageCounterText.setText(countAge);
